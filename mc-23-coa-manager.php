                ...
                <tbody>
                    ...
                    ?>
                        <tr class="coa-row"
                            ...
                            <td class="col-balance"><?php echo number_format(floatval(isset($c->balance) ? $c->balance : 0), 2, ',', '.'); ?></td>
        ...
        <div class="puri-modal-content">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="1" name="balance" id="inp_balance" class="full-width" placeholder="0.00">
                </div>
...
*/
        ...
        .badge-revenue { background:#bdfdcd; color:#166534; border:1px solid #166534; border-radius:20px 20px 20px 0;}
        .badge-expense { background:#fee2e2; color:#991b1b; border:1px solid #fca5a5; border-radius:20px 20px 0px 20px ;}

		.col-balance { text-align: right; font-size: 14px; }
...
function openModal(mode, btn = null) {
    ...
    // 1. Reset: Tetap kosongkan agar placeholder di HTML muncul saat 'Create'
    inpCode.value = ''; 
    inpOldCode.value = ''; 
    inpId.value = ''; 
    inpName.value = ''; 
    inpType.value = 'ASSET'; 
    inpCash.checked = false; 
    inpBalance.value = ''; // Mengosongkan value memicu munculnya placeholder
    inpDesc.value = '';
    ...
    if (mode === 'create') {
        ...
        //---- deprecated : inpBalance.value = row.dataset.balance || '';
		// FORMAT 2 DESIMAL DI SINI
        if (row.dataset.balance) {
            inpBalance.value = parseFloat(row.dataset.balance).toFixed(2);
        } else {
            inpBalance.value = '';
        }
        inpDesc.value = row.dataset.desc || '';

        // 2. Modifikasi Edit: Auto-select teks saldo agar langsung bisa ditimpa
        setTimeout(() => { inpBalance.select(); }, 100);
        ...
        // 3. Modifikasi Duplicate: Jika ada saldo awal bawaan, langsung seleksi juga
        //--- deprecated: inpBalance.value = row.dataset.balance || '';
