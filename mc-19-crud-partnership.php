...
function puri_partnership_harmony_logic($post_id) {
	...
	if ($id_type === 'KTP') {
        if (strlen(preg_replace('/\D/', '', $nik)) !== 16) $missing[] = 'NIK KTP (Harus 16 Digit)';
    } else {
...
// Pastikan aturan penamaan file berlaku saat upload di Admin mc-19
add_filter('wp_handle_upload_prefilter', function($file) {
    if (isset($_POST['post_id']) && get_post_type($_POST['post_id']) === 'pr_customer') {
        $post_id = $_POST['post_id'];
        
        $id_type   = get_field('cust_id_type', $post_id) ?: 'ID';
        $cust_name = sanitize_title(get_the_title($post_id));
        $cust_id   = get_field('cust_nik', $post_id) ?: '000000';
